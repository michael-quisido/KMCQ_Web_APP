...
export default function SubpageEditor() {
  ...
  useEffect(() => {
    if (!slug) return;
    const pageTitle = subpageTitles[slug] || slug;
    setTitle(pageTitle);
    fetch("/api/custom-pages").then(r => r.json()).then(async (pages: { id: number; slug: string }[]) => {
      ...
      if (page) {
        ...
        const res = await fetch("/api/custom-pages", {
          ...
          body: JSON.stringify({ slug, title: pageTitle, content: buildDefaultContent(pageTitle) }),
        });
        if (res.ok) {
